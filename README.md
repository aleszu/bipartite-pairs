...
```
where:
    *  `adj_matrix.mtx.gz` is a file containing the adjacency matrix of an undirected bipartite graph, in [matrix market](https://math.nist.gov/MatrixMarket/formats.html) format. (Many languages, including python and R, have routines to read and write this format.)
 The graph will be treated as unweighted.
...
* **Code walk-through when computing the similarity method `weighted_corr_exp`**. A stack trace would show something like the following functions called.
 ...
 1. Compute `weighted_corr_exp` as a dot product:
    ...
    ```   
 (Numbering should continue from 9, but I don't know how to make markdown support it. :( )
