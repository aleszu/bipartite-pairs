...
def score_pairs(pairs_generator, adj_matrix, which_methods, print_timing=False,
    ...
    if 'sharedSize' in which_methods:
        scores['sharedSize'] = compute_scores_with_transform(pairs_generator, adj_matrix,
                                                             shared_size_transform,
                                                             print_timing=print_timing)
        if run_all_implementations:
            scores['sharedSize'] = compute_scores_orig(pairs_generator(adj_matrix), extra_implementations.shared_size,
                                                       print_timing=print_timing,
    ...
    if 'weighted_corr' in which_methods:
        if sparse.isspmatrix(adj_matrix):
            ...
            scores['weighted_corr'] = scoring_methods_fast.simple_weighted_corr_sparse(pairs_generator, adj_matrix,
                                                                                       pi_vector=all_named_args['pi_vector'],
                                                                                       print_timing=print_timing)
        else:
            scores['weighted_corr'] = scoring_methods_fast.simple_only_weighted_corr(pairs_generator, adj_matrix,
                                                                                     pi_vector=all_named_args['pi_vector'],
        ...
        if run_all_implementations:
            ...
            scores['weighted_corr'] = compute_scores_from_terms(pairs_generator, adj_matrix, extra_implementations.wc_terms,
                                                                pi_vector=all_named_args['pi_vector'],
    ...
    if 'adamic_adar' in which_methods:
        ...
        if run_all_implementations:
            ...
            scores['adamic_adar'] = compute_scores_with_transform(pairs_generator, adj_matrix,
                                                                  extra_implementations.adamic_adar_transform,
...
def compute_scores_orig(pairs_generator, sim_func, print_timing=False, **named_args_to_func):
    ...
    print "original " + sim_func.__name__ + ": " + str(end - start) + " secs" if print_timing else ''
...
def compute_scores_from_terms(pairs_generator, adj_matrix, scores_bi_fun, print_timing=False, **named_args_to_func):
    start = timer()
    terms_for_11, value_10, terms_for_00 = scores_bi_fun(**named_args_to_func)
    scores = []
    ...
    print scores_bi_fun.__name__ + ": " + str(end - start) + " secs" if print_timing else ''
    return scores
...
#  but this may change on different sized data sets, and depending on whether matrix is sparse or dense.
